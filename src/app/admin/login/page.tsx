...
export default function AdminLogin() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    if (error) {
      setForgotPasswordMessage(`Xəta: ${error.message}`);
    } else {
      setForgotPasswordMessage('Şifrə yeniləmə linki emailinizə göndərildi!');
    }
  };
        ...
        {!showForgotPassword ? (
          <>
            <div className="mb-6 text-center">
              ...
              <p className="text-sm text-indigo-900/70 mt-1">Hesab məlumatlarınızla daxil olun</p>
            </div>
            <form onSubmit={signIn} className="space-y-4">
              ...
              <div>
                ...
                <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" required className="w-full rounded-lg border border-indigo-200 p-3" placeholder="••••••••" />
              </div>
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4">
            <div className="text-center mb-4">
              <h2 className="text-lg font-semibold text-indigo-900">Şifrəni Yenilə</h2>
              <p className="text-sm text-indigo-900/70 mt-1">Emailinizə şifrə yeniləmə linki göndərəcəyik</p>
            </div>
            <div>
            ...
            {forgotPasswordMessage && (
              <p className={`text-sm ${forgotPasswordMessage.includes('Xəta') ? 'text-red-600' : 'text-green-600'}`}>
                {forgotPasswordMessage}
              </p>
            )}
            <div className="flex gap-3">
              ...
              >
                {forgotPasswordLoading ? 'Göndərilir...' : 'Link Göndər'}
              </motion.button>
              <motion.button
                whileHover={{ scale: 1.01 }}
                whileTap={{ scale: 0.98 }}
